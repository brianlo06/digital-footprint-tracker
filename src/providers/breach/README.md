...
**Current implementation:** a zero-network synthetic adapter covers bounded success, empty, duplicate, malformed, hostile, schema-change, timeout, authentication, rate-limit, outage, and pagination behavior. It accepts only an opaque identifier UUID with the `VERIFIED_EMAIL_SELF` scope, costs zero units, and emits synthetic normalized candidates. A non-routed invocation service also requires exact ownership, active state, verification no more than 24 hours old, purpose-specific consent, and a successful fail-closed request/cost reservation.
