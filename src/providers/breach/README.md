...
**Current implementation:** a zero-network synthetic adapter covers bounded success, empty, duplicate, malformed, hostile, schema-change, timeout, authentication, rate-limit, outage, and pagination behavior. It accepts only an opaque identifier UUID with the `VERIFIED_EMAIL_SELF` scope, costs zero units, and emits synthetic normalized candidates.
