...
- `security/`: encryption, key-rewrap policy, auth fail-closed behavior, field-level PII log canaries, centralized-sink enforcement across the application and standalone Workers, trusted network parsing, verification-gateway isolation, and the closed provider boundary.
