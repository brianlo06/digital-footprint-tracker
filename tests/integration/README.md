...
These tests use an explicitly supplied disposable local PostgreSQL database and synthetic `.test` identifiers. They cover account/identifier lifecycle, concurrent onboarding, cross-account denial, verification lockout, destructive-action reauthentication, deletion failure/retry quarantine, bounded retention maintenance, and restart-safe envelope key rewrap/rollback.
