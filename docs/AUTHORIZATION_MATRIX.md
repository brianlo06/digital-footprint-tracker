...
**Scope:** Current account, email-identifier, verification, and deletion foundation only

Every Server Action is treated as a directly reachable mutation endpoint. Layout checks improve navigation behavior but are not an authorization boundary. Leaf pages and data-access services re-check the current principal near protected data. Each action also consumes database-atomic user and network limits before form validation; throttling is defense in depth, not authorization.
...
| `/settings/privacy` | Current principal plus active account in the leaf page | Redirect to onboarding                | Generic policy text; no plaintext identifiers          |
