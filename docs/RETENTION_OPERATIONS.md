...
| Aged observation                | Older than the observation retention period **and not the newest observation for its finding** | delete permanently; the finding row and its most recent observation always survive                                                                                     |

Failed, requested, or auth-revoked deletion receipts are deliberately retained because they may represent unfinished deletion work. Active-user audit events are not touched. Verification history remains until account deletion, but the challenge secret is destroyed at expiry. Pending and claimed scan jobs are never touched: only terminal job detail ages out, matching the completed scan/job row of the privacy retention table, and the user-visible scan history summary is unaffected. Observation history ages out at 24 months, but every finding keeps its most recent observation however old it is: `docs/PRIVACY.md` retains evidence summary and provenance for the life of the finding, so a finding must never be left with nothing explaining its current state. Findings themselves are never deleted by retention.
...
- PostgreSQL rejects null/unbounded batches, clocks more than five minutes in the future, orphan-audit or scan-job cutoffs newer than the one-day minimum, and observation cutoffs newer than the thirty-day minimum.
