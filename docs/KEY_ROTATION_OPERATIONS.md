...
Lookup-key rotation remains deliberately blocked. Because deterministic tokens must be recomputed from normalized identifier values, it requires a separately approved, auditable procedure with explicit plaintext-handling scope, uniqueness-conflict handling, dual-read/cutover semantics, recovery, and deletion guarantees.
