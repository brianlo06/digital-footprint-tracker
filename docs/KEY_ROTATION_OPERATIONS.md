...
Lookup-key rotation remains deliberately blocked from implementation, but its multi-domain cutover and recovery design is now explicit in [ADR 0016](adr/0016-lookup-key-rotation.md). The proposal covers controlled identifier plaintext handling, versioned token rows, incomplete/completed deletion receipts, atomic dual-key rate limiting, uniqueness conflicts, backup compatibility, rollback, deletion races, and irreversible old-key destruction. It requires owner/security approval plus an approved KMS and operational authority before code or schema work begins.
