...
- a pure core/service split (`delivery-outbox-core.ts`, `delivery-outbox-service.ts`, `delivery-worker-core.ts`) and a demonstration `workers/verification-delivery.ts` that claims, strictly validates after decryption, permanently rejects poison commands without provider access, preserves ciphertext on key/decryption failure, and hands valid deliveries to a `DeliveryProvider` — today only `SyntheticNoopDeliveryProvider`, which always succeeds and sends nothing;
...
| 5   | Concurrent integration tests for atomic enqueue, duplicate claims, lease expiry, retries, expiry/completion/revocation, deletion races, and payload destruction | **Done (local)** — see the verification-evidence bullets in `PHASE_1_STATUS.md`; unit tests also cover strict command decoding, poison commands, recoverable decryption failure, the default-on kill switch, every bounded provider outcome, and thrown-provider transient retry; provider-specific uncertain-response behavior awaits a chosen provider |
| 6   | Canaries proving no destination, code, content, ciphertext, provider credential, or response can reach telemetry                                                | Partial local evidence — generic sensitive-value canaries cover these value classes, the delivery Worker is included in the no-bypass source scan, and every Worker config disables invocation logs/traces; a hosted end-to-end delivery canary remains required                                                                                         |
